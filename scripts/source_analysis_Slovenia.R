...
genes_data = read_csv('Oncobox_Slovenia_primary_GBM_PFS_CNR.csv')
...
dat_oncobox <- read_delim("D:/Oncobox/LTS_vs_STS_manuscript/LTS_vs_STS_manuscript/exp_data/Oncobox_Slovenia_exp_matrix.txt",
        "\t", escape_double = FALSE, trim_ws = TRUE)
our_genes <- dat_oncobox$SYMBOL
...
# For Slovenia data
annotation = read_excel("KopijaAB_Samples for NGS 14.03.2019-2_ENG - joint Oncobox Slovenia samples.xlsx", sheet = 2)
# annotation = na.omit(annotation[, c("Oncobox ID", "Survival (months)")])
annotation = na.omit(annotation[, c("Oncobox ID", 'TTP (months)')])
colnames(annotation)[2] = 'survival months'
annotation = annotation[(annotation$`survival months` != '/') & (annotation$`survival months` != 'FALSE') & (annotation$`survival months` != 'survival months'),]
...
colnames(dat_oncobox) <- c("SYMBOL", row.names(genes_data))
dat_oncobox <- as.data.frame(dat_oncobox)
rownames(dat_oncobox) <- dat_oncobox$SYMBOL
...
HR_p = c()
i = 1
for (gene in rownames(dat_oncobox)) {
  # gene=colnames(genes_data)[1]
  print(i)
  i = i + 1
  dat2 = data.frame(bcr_patient_barcode = annotation$`Oncobox ID`,
                  time = annotation$`survival months`,
                  CNR = dat_oncobox[gene,], status = 1,
                  Censored = 1)
  dat2 = dat2[!duplicated(dat2),]
  colnames(dat2)[2] = 'time'
  dat2$time = as.numeric(as.character(dat2$time))
...
write.xlsx(final, 'Slovenia_primary_GBM_PFS_HRs_genes_third - test - Oncobox Slovenia samples.xlsx')
...
dat2 = data.frame(bcr_patient_barcode = annotation$`Oncobox ID`, # for Slovenia data
                time = annotation$`survival months`,
                CNR = as.numeric(dat_oncobox["CRNDE", annotation$`Oncobox ID`]), status = 1,
                Censored = 1)
dat2 <- dat2[!duplicated(dat2),]
colnames(dat2)[2] <- 'time'
dat2$time <- as.numeric(as.character(dat2$time))
median <- median(dat2$CNR)
dat2 <- dat2[(dat2$CNR >= quantile(dat2$CNR, 0.6)) | (dat2$CNR <= quantile(dat2$CNR, 0.3)),]

fit <- survfit(Surv(time = dat2$time, event = dat2$Censored) ~ dat2$CNR > median, data = dat2)

# pdf(file = "CRNDE_Slovenia_KM_PFS.pdf")
pdf(file = "CRNDE_Oncobox_Slovenia_KM_PFS - QC passed.pdf")
ggsurvplot(fit,
           pval = TRUE,
           pval.method = TRUE,
           size = 2,
...
pdf(file = "CRNDE_Oncobox_Slovenia_forest_PFS - QC passed.pdf", width = 7, height = 1.5)
ggforest(model)
dev.off()
