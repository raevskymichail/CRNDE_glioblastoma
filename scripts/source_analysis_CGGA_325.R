...
CGGA_annotation <- read_tsv("CGGA.mRNAseq_325_clinical.20200506.txt")
...
counts <- read_tsv("CGGA.mRNAseq_325.RSEM-genes.20200506.txt")
...
# write.csv(counts_lgg, "expression_CGGA_325.csv", row.names = TRUE)
#counts_lgg=read_csv("counts_lgg.csv")
...
write.csv(counts_lgg, 'counts_CGGA_325_GBM_OS.csv', row.names = TRUE)
a <- read_csv('counts_CGGA_325_GBM_OS.csv')
...
counts_lgg$SYMBOL = row.names(counts_lgg)
counts_lgg = counts_lgg[, c("SYMBOL", colnames(counts_lgg)[1:(ncol(counts_lgg) - 1)])]
write.csv(counts_lgg, 'CGGA_325_counts_primary_GBM_MGMT_met_IDH_WT_OS_pathways.csv', row.names = FALSE)
...
colnames(pathways) <- c("SYMBOL", "Tumour_Bad", "Norm_Good")
write.csv(pathways, 'CGGA_325_counts_primary_GBM_OS_pathways_good_bad.csv', row.names = FALSE)
...
CGGA = read_csv('counts_CGGA_325_GBM_OS.csv')
