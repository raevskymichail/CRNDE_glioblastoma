...
dat_TCGA = read.csv('C:/Users/raevs/Desktop/Oncobox/LTS_vs_STS_manuscript/gliomas_pathways_PFS/TCGA_read_counts.csv', row.names = 1)
...
####################################
## Survival Analysis for Pathways ##
####################################

# pathways = read_excel('Mainz_primary_GBM_PFS.xlsx', sheet = 2)
# pathways = read_excel('Slovenia_primary_GBM_OS.xlsx', sheet = 2)
# pathways = read_excel('Slovenia_primary_GBM_PFS_PAS2.xlsx')
pathways = read_csv('CGGA_325_primary_GBM_OS_PAS2.csv')
# pathways = read_csv('CGGA_693_primary_GBM_OS_PAS2.csv')
...
pathways$bcr_patient_barcode <- row.names(pathways)
...
row.names(pathways) = p
row.names(pathways) <- gsub("tumour_", "", row.names(pathways))

# intersect(row.names(pathways), as.character(data_lgg$bcr_patient_barcode))

library("survival")
library("survminer")


# data_lgg$`Patient ID` <- data_lgg$bcr_patient_barcode
CGGA_annotation_B <- read_tsv('CGGA.mRNAseq_693_clinical.20200506.txt')
...
CGGA_annotation <- CGGA_annotation[CGGA_annotation$Histology == 'GBM',]

CGGA_annotation <- na.omit(CGGA_annotation[CGGA_annotation$CGGA_ID %in% row.names(pathways), c("CGGA_ID", "OS")])
...
annotation <- annotation[intersect(row.names(annotation), gsub("tumour_", "", row.names(pathways))),]
...
for (pathway in colnames(pathways)) {
  ...
  i = i + 1
  #pathway='CRNDE'
  # dat2 = data.frame(bcr_patient_barcode = CGGA_annotation$CGGA_ID,
  #                 time = CGGA_annotation$OS,
  #                 CNR = pathways[as.character(CGGA_annotation$CGGA_ID), pathway], status = 1,
  #                 Censored = 1)
  dat2 = data.frame(bcr_patient_barcode = annotation$`Oncobox ID`, # for Slovenia data
                  time = annotation$`survival months`,
                  CNR = pathways[as.character(annotation$`Oncobox ID`), pathway], status = 1,
                  Censored = 1)
  # dat2 = data.frame(bcr_patient_barcode = data_lgg$`Patient ID`,
  #                 time = data_lgg$death_days_to,
  #                 CNR = pathways[as.character(data_lgg$`Patient ID`), pathway], status = 1,
  #                 Censored = 1)

  #dat2=data.frame(bcr_patient_barcode=row.names(pathways),
  #                time=TTP[row.names(pathways),][,2],
  #                CNR=pathways[,pathway],status=1,
  #                Censored=1)
  ...
  median = median(dat2$CNR)
  dat2$time = dat2$time / 30
  dat2 = dat2[(dat2$CNR >= quantile(dat2$CNR, 0.6667)) | (dat2$CNR <= quantile(dat2$CNR, 0.33333)),]
  res.cox <- coxph(Surv(time = dat2$time, event = dat2$Censored) ~ dat2$CNR > median, data = dat2)
...
library(venn)
venn(list(HR = final[final$HR_p < 0.05, 'pathways'], log_rank = final[final$log_rank < 0.05, 'pathways']))
...
write.xlsx(final, 'CGGA_325_OS_HRs_pathways_third.xlsx')
# write.xlsx(final, 'CGGA_693_OS_HRs_pathways_third.xlsx')
write.xlsx(final, 'Slovenia_primary_GBM_OS_HRs_pathways_third - test.xlsx')
# write.xlsx(final, 'Slovenia_primary_GBM_PFS_HRs_pathways_third - test.xlsx')

final_old = read_excel('GBM_OS_HRs_pathways.xlsx')

fit <- survfit(Surv(time = dat2$time, event = dat2$Censored) ~ dat2$CNR > median, data = dat2)
...
dev.off()
dat2$Status = as.numeric(dat2$CNR > median)
...
# genes_data = read_excel('Mainz_primary_GBM_PFS.xlsx', sheet = 4) # we need CNR
# genes_data = read_excel('lgg_pfs.xlsx', sheet = 4) # we need TCGA CNR
# genes_data = read_excel('Slovenia_primary_GBM_PFS_CNR.xlsx')
# genes_data = read_excel('Slovenia_primary_gbm_os.xlsx', sheet = 4)
genes_data = read_csv('CGGA_325_primary_GBM_OS_CNR.csv')
# genes_data = read_csv('CGGA_693_primary_GBM_OS_CNR.csv')
genes_data$index <- NULL

dat_oncobox <- read_delim("C:/Users/raevs/Desktop/Oncobox/LTS_vs_STS_manuscript/exp_data/slovenia_exp_matrix.txt",
        "\t", escape_double = FALSE, trim_ws = TRUE)
our_genes <- dat_oncobox$SYMBOL

genes_data = genes_data[!is.na(match(genes_data$SYMBOL, our_genes)),]
p = genes_data$SYMBOL
genes_data$SYMBOL = NULL
genes_data = as.data.frame(genes_data)
row.names(genes_data) = p
# colnames(genes_data) = str_sub(colnames(genes_data), 8, -1L)
genes_data = as.data.frame(t(genes_data))

# genes_data=genes_data-1
# genes_data$bcr_patient_barcode = c(str_sub(row.names(genes_data), 1, 3))
# genes_data$bcr_patient_barcode = c(str_sub(row.names(genes_data), 1, 12))
genes_data$bcr_patient_barcode <- row.names(genes_data)
genes_data = aggregate(. ~ bcr_patient_barcode, data = genes_data, FUN = mean)
p = genes_data$bcr_patient_barcode
genes_data$bcr_patient_barcode = NULL
row.names(genes_data) = p
row.names(genes_data) <- gsub("tumour_", "", row.names(genes_data))

# intersect(row.names(genes_data), as.character(data_lgg$sample_id))
...
CGGA_annotation <- na.omit(CGGA_annotation[CGGA_annotation$CGGA_ID %in% row.names(genes_data), c("CGGA_ID", "OS")])

# For Slovenia data
annotation = read_excel("KopijaAB_Samples for NGS 14.03.2019-2_ENG PFS.xlsx", sheet = 2)
annotation = na.omit(annotation[, c("Oncobox ID", "Survival (months)")])
# annotation = na.omit(annotation[,c("Oncobox ID",'TTP (months)')])
colnames(annotation)[2] = 'survival months'
annotation = annotation[(annotation$`survival months` != '/') & (annotation$`survival months` != 'FALSE') & (annotation$`survival months` != 'survival months'),]
...
annotation <- as.data.frame(annotation)
row.names(annotation) = annotation$`Oncobox ID`
annotation <- annotation[intersect(row.names(annotation), gsub("tumour_", "", row.names(genes_data))),]

HRs = c()
log_rank = c()
HR_p = c()
i = 1
for (gene in colnames(genes_data)) {
  # gene=colnames(genes_data)[1]
  print(i)
  i = i + 1
  # gene='CRNDE'
  # dat2 = data.frame(bcr_patient_barcode = CGGA_annotation$CGGA_ID,
  #                 time = CGGA_annotation$OS,
  #                 CNR = genes_data[as.character(CGGA_annotation$CGGA_ID), gene], status = 1,
  #                 Censored = 1)
  dat2 = data.frame(bcr_patient_barcode = annotation$`Oncobox ID`, # for Slovenia data
                  time = annotation$`survival months`,
                  CNR = genes_data[as.character(annotation$`Oncobox ID`), gene], status = 1,
                  Censored = 1)
  # dat2 = data.frame(bcr_patient_barcode = data_lgg$`Patient ID`,
  #                 time = data_lgg$death_days_to,
  #                 CNR = genes_data[as.character(data_lgg$`Patient ID`), gene], status = 1,
  #                 Censored = 1)

  #dat2=data.frame(bcr_patient_barcode=row.names(genes_data),
  #                time=TTP[row.names(genes_data),][,2],
  #                CNR=genes_data[,gene],status=1,
  #                Censored=1)
  dat2 = dat2[!duplicated(dat2),]
  colnames(dat2)[2] = 'time'
  dat2$time = as.numeric(as.character(dat2$time))
  ...
  dat2 = dat2[(dat2$CNR >= quantile(dat2$CNR, 0.6667)) | (dat2$CNR <= quantile(dat2$CNR, 0.33333)),]
  res.cox <- coxph(Surv(time = dat2$time, event = dat2$Censored) ~ dat2$CNR > median, data = dat2)
  HRs[gene] = res.cox$coefficients
  a = summary(res.cox)
  log_rank[gene] = a$logtest[3]
  HR_p[gene] = a$coefficients[5]
}
...
# write.xlsx(final, 'Mainz_PFS_HRs_pathways_third.xlsx')
# write.xlsx(final, 'Mainz_PFS_HRs_pathways_third.xlsx')
write.xlsx(final, 'Slovenia_primary_GBM_OS_HRs_genes_third - test.xlsx')
# write.xlsx(final, 'Slovenia_primary_GBM_PFS_HRs_genes_third - test.xlsx')
# write.xlsx(final, 'CGGA_693_OS_HRs_genes_third.xlsx')

# final_old = read_excel('GBM_OS_HRs_pathways.xlsx')
final = read_excel('Slovenia_primary_GBM_OS_HRs_pathways.xlsx')

# KM plots

dat2 = data.frame(bcr_patient_barcode = CGGA_annotation$CGGA_ID,
                time = CGGA_annotation$OS,
                CNR = genes_data[as.character(CGGA_annotation$CGGA_ID), "CRNDE"], status = 1,
                Censored = 1)
dat2 = data.frame(bcr_patient_barcode = annotation$`Oncobox ID`, # for Slovenia data
                time = annotation$`survival months`,
                CNR = genes_data[as.character(annotation$`Oncobox ID`), "CRNDE"], status = 1,
                Censored = 1)
# dat2 = data.frame(bcr_patient_barcode = data_lgg$sample_id,
#                 time = data_lgg$death_days_to,
#                 CNR = genes_data[as.character(data_lgg$sample_id), "CRNDE"], status = 1,
#                 Censored = 1)
# saveRDS(dat2, "TCGA_dat2_PFS.rds")
# dat2 = data.frame(bcr_patient_barcode = data_lgg$bcr_patient_barcode,
#                time = data_lgg$death_days_to,
#                CNR = genes_data[as.character(data_lgg$bcr_patient_barcode), "CRNDE"], status = 1,
#                Censored = 1)
# saveRDS(dat2, "Slovenia_dat2_PFS.rds")
dat2 <- dat2[!duplicated(dat2),]
colnames(dat2)[2] <- 'time'
dat2$time <- as.numeric(as.character(dat2$time))
dat2 <- aggregate(. ~ bcr_patient_barcode, data = dat2, FUN = min)
median <- median(dat2$CNR)
# dat2$time <- dat2$time / 30 # IMPORTANT - for Slovenia hiden
dat2 <- dat2[(dat2$CNR >= quantile(dat2$CNR, 0.6667)) | (dat2$CNR <= quantile(dat2$CNR, 0.33333)),]

fit <- survfit(Surv(time = dat2$time, event = dat2$Censored) ~ dat2$CNR > median, data = dat2)

pdf(file = "CRNDE_CGGA_325_KM_OS.pdf")
# pdf(file = "CRNDE_CGGA_693_KM_OS.pdf")
# pdf(file = "CRNDE_TGGA_KM_PFS.pdf")
# pdf(file = "CRNDE_Slovenia_KM_PFS.pdf")
pdf(file = "CRNDE_Slovenia_KM_PFS - QC passed.pdf")
ggsurvplot(fit,
           pval = TRUE,
           pval.method = TRUE,
           size = 2,
#  legend.labs = c("lower than 1/3", "higher than 1/3"),
           legend.labs = c("low CRNDE", "high CRNDE"),
           pval.size = 7,
           font.x = 20,
           font.y = 20,
           font.tickslab = 18,
...
dat2$Status = as.numeric(dat2$CNR > median)
model <- coxph(Surv(time = dat2$time, event = dat2$Censored) ~ dat2$Status, data = dat2[, c('time', 'Censored', 'Status')])
# pdf(file = "CRNDE_CGGA_325_forest_OS.pdf", width = 7, height = 1.5)
# pdf(file = "CRNDE_CGGA_693_forest_OS.pdf", width = 7, height = 1.5)
# pdf(file = "CRNDE_TCGA_forest_PFS.pdf", width = 7, height = 1.5)
# pdf(file = "CRNDE_Slovenia_forest_PFS.pdf", width = 7, height = 1.5)
pdf(file = "CRNDE_Slovenia_forest_PFS - QC passed.pdf", width = 7, height = 1.5)
ggforest(model)
dev.off()
...
counts_B <- read_tsv("CGGA.mRNAseq_693.RSEM-genes.20200506.txt")
...
write.csv(counts_lgg, 'expression_CGGA.csv', row.names = TRUE)
#counts_lgg=read_csv('counts_lgg.csv')
...
write.csv(counts_lgg, 'counts_CGGA_GBM_OS.csv', row.names = TRUE)
a = read_csv('counts_CGGA_GBM_OS.csv')
...
counts_lgg$SYMBOL = row.names(counts_lgg)
counts_lgg = counts_lgg[, c("SYMBOL", colnames(counts_lgg)[1:(ncol(counts_lgg) - 1)])]
write.csv(counts_lgg, 'CGGA_counts_primary_GBM_MGMT_met_IDH_WT_OS_pathways.csv', row.names = FALSE)
...
colnames(pathways) = c("SYMBOL", "Tumour_Bad", "Norm_Good")
write.csv(pathways, 'CGGA_counts_primary_GBM_OS_pathways_good_bad.csv', row.names = FALSE)
...
pathways = as.data.frame(cbind(row.names(counts_lgg), 10 ** rowMeans(log10(counts_lgg[, data_lgg[data_lgg$death_days_to < median(data_lgg$death_days_to), "bcr_patient_barcode"]$bcr_patient_barcode])),
                             10 ** rowMeans(log10(counts_lgg[, data_lgg[data_lgg$death_days_to > median(data_lgg$death_days_to), "bcr_patient_barcode"]$bcr_patient_barcode]))))
# pathways = as.data.frame(cbind(row.names(counts_lgg), 10 ** rowMeans(log10(counts_lgg[, data_lgg[data_lgg$death_days_to < median(data_lgg$death_days_to), "bcr_patient_barcode"]])),
#                              10 ** rowMeans(log10(counts_lgg[, data_lgg[data_lgg$death_days_to > median(data_lgg$death_days_to), "bcr_patient_barcode"]]))))
colnames(pathways) = c("SYMBOL", "Tumour_Bad", "Norm_Good")
...
final_TCGA_OS = read_excel('GBM_OS_HRs_genes_third.xlsx')
final_TCGA_OS = na.omit(final_TCGA_OS)
# final_CGGA_OS = read_excel('CGGA_primary_GBM_OS_HRs_genes_third.xlsx')
# final_CGGA_OS = na.omit(final_CGGA_OS)
final_CGGA_325_OS = read_excel('CGGA_325_OS_HRs_genes_third.xlsx')
final_CGGA_325_OS = na.omit(final_CGGA_325_OS)
final_CGGA_693_OS = read_excel('CGGA_693_OS_HRs_genes_third.xlsx')
final_CGGA_693_OS = na.omit(final_CGGA_693_OS)
# final_Slovenia_OS = read_excel('Slovenia_primary_GBM_PFS_HRs_genes_third.xlsx')
final_Slovenia_OS = read_excel('Slovenia_primary_GBM_OS_HRs_genes_third.xlsx')
# final_Slovenia_OS = read_excel('Slovenia_primary_GBM_OS_HRs_genes_third - test.xlsx')
# final_Slovenia_OS = read_excel('Slovenia_primary_GBM_PFS_HRs_genes_third - test.xlsx')
...
# a = read_excel('counts_gbm_PFS_pathways_good_bad.xlsx', sheet = 4)
a = read_excel('counts_gbm_OS_pathways_good_bad.xlsx', sheet = 4)
a$index = NULL
colnames(a)[1] = 'gene'
final_TCGA_OS = left_join(final_TCGA_OS, a, by = 'gene')
...
# a$index = NULL
# colnames(a)[1] = 'gene'
# final_CGGA_OS = left_join(final_CGGA_OS, a, by = 'gene')
...
a = read_excel('CGGA_325_counts_primary_GBM_OS_pathways_good_bad.xlsx')
colnames(a)[1] = 'gene'
final_CGGA_325_OS = left_join(final_CGGA_325_OS, a, by = 'gene')
final_CGGA_325_OS = final_CGGA_325_OS[(final_CGGA_325_OS$log_rank < 0.05) & (final_CGGA_325_OS$HR_p < 0.05),]
final_CGGA_325_OS = final_CGGA_325_OS[order(final_CGGA_325_OS$Tumour_Bad),]
# final_CGGA_325_OS = rbind(final_CGGA_325_OS[1:10,], final_CGGA_325_OS[(nrow(final_CGGA_325_OS) - 9):nrow(final_CGGA_325_OS),])
...
colnames(a)[1] = 'gene'
final_CGGA_693_OS = left_join(final_CGGA_693_OS, a, by = 'gene')
final_CGGA_693_OS = final_CGGA_693_OS[(final_CGGA_693_OS$log_rank < 0.05) & (final_CGGA_693_OS$HR_p < 0.05),]
final_CGGA_693_OS = final_CGGA_693_OS[order(final_CGGA_693_OS$Tumour_Bad),]
# final_CGGA_693_OS = rbind(final_CGGA_693_OS[1:10,], final_CGGA_693_OS[(nrow(final_CGGA_693_OS) - 9):nrow(final_CGGA_693_OS),])

# # a = read_excel('Mainz_counts_primary_GBM_PFS_pathways_good_bad.xlsx')
# a = read_excel('Mainz_counts_primary_GBM_OS_pathways_good_bad.xlsx')
# colnames(a)[1] = 'gene'
# final_Mainz_OS = left_join(final_Mainz_OS, a, by = 'gene')
# final_Mainz_OS = final_Mainz_OS[(final_Mainz_OS$log_rank < 0.05) & (final_Mainz_OS$HR_p < 0.05),]
# final_Mainz_OS = na.omit(final_Mainz_OS[order(final_Mainz_OS$Tumour_Bad),])
# final_Mainz_OS = rbind(final_Mainz_OS[1:10,], final_Mainz_OS[(nrow(final_Mainz_OS) - 9):nrow(final_Mainz_OS),])

# a = read_excel('Slovenia_counts_primary_GBM_PFS_pathways_good_bad.xlsx', sheet = 4)
a = read_excel('Slovenia_counts_primary_GBM_OS_pathways_good_bad.xlsx', sheet = 4)
a$index = NULL
colnames(a)[1] = 'gene'
final_Slovenia_OS = left_join(final_Slovenia_OS, a, by = 'gene')
...
genes_CGGA_325 = final_CGGA_325_OS$gene
genes_CGGA_693 = final_CGGA_693_OS$gene
...
# my_list_plus = list(TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$Tumour_Bad > 0), 'SYMBOL']$SYMBOL,
#                   CGGA_325_OS = final_CGGA_325_OS[(final_CGGA_325_OS$Tumour_Bad > 0), 'SYMBOL']$SYMBOL,
#                   CGGA_693_OS = final_CGGA_693_OS[(final_CGGA_693_OS$Tumour_Bad > 0), 'SYMBOL']$SYMBOL,
#                   Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$Tumour_Bad > 0), 'SYMBOL']$SYMBOL)
# my_list_minus = list(TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$Tumour_Bad < 0), 'SYMBOL']$SYMBOL,
#                    CGGA_325_OS = final_CGGA_325_OS[(final_CGGA_325_OS$Tumour_Bad < 0), 'SYMBOL']$SYMBOL,
#                    CGGA_693_OS = final_CGGA_693_OS[(final_CGGA_693_OS$Tumour_Bad < 0), 'SYMBOL']$SYMBOL,
#                    Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$Tumour_Bad < 0), 'SYMBOL']$SYMBOL)
...
my_list_plus = list(CGGA_325_GBM_OS = final_CGGA_325_OS[(final_CGGA_325_OS$log_rank < 0.05) & (final_CGGA_325_OS$HR_p < 0.05) & (final_CGGA_325_OS$HR > 0), 'gene']$gene,
                  CGGA_693_GBM_OS = final_CGGA_693_OS[(final_CGGA_693_OS$log_rank < 0.05) & (final_CGGA_693_OS$HR_p < 0.05) & (final_CGGA_693_OS$HR > 0), 'gene']$gene,
                   ...
                   TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$log_rank < 0.05) & (final_TCGA_OS$HR_p < 0.05) & (final_TCGA_OS$HR > 0), 'gene']$gene)
my_list_minus = list(CGGA_325_GBM_OS = final_CGGA_325_OS[(final_CGGA_325_OS$log_rank < 0.05) & (final_CGGA_325_OS$HR_p < 0.05) & (final_CGGA_325_OS$HR < 0), 'gene']$gene,
                   CGGA_693_GBM_OS = final_CGGA_693_OS[(final_CGGA_693_OS$log_rank < 0.05) & (final_CGGA_693_OS$HR_p < 0.05) & (final_CGGA_693_OS$HR < 0), 'gene']$gene,
                   Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR_p < 0.05) & (final_Slovenia_OS$HR < 0), 'gene']$gene,
                   TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$log_rank < 0.05) & (final_TCGA_OS$HR_p < 0.05) & (final_TCGA_OS$HR < 0), 'gene']$gene)
...
#                   CGGA_GBM_OS = final_CGGA_OS[(final_CGGA_OS$log_rank < 0.05) & (final_CGGA_OS$HR_p < 0.05) & (final_CGGA_OS$HR > 0), 'gene']$gene,
#                    Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR_p < 0.05) & (final_Slovenia_OS$HR > 0), 'gene']$gene)
...
my_list_all = list(TCGA_GBM_OS = c(my_list_plus$final_TCGA_OS, my_list_minus$final_TCGA_OS),
                  CGGA_325 = c(my_list_plus$CGGA_325_OS, my_list_minus$CGGA_325_OS),
                 CGGA_693 = c(my_list_plus$CGGA_693_OS, my_list_minus$CGGA_693_OS),
                 Slovenia = c(my_list_plus$Slovenia_GBM_OS, my_list_minus$Slovenia))

# my_list_all = list(TCGA = c(my_list_plus$TCGA_GBM_OS, my_list_minus$TCGA_GBM_OS),
#                  CGGA_GBM = c(my_list_plus$CGGA_GBM_OS, my_list_minus$CGGA_GBM_OS),
#                  Slovenia = c(my_list_plus$Slovenia_GBM_OS, my_list_minus$Slovenia_GBM_OS))
saveRDS(my_list_all, "GBM_common_OS_plus_minus_genes_CGGA_325_693_TCGA_Slovenia.rds")
# my_list_all <- readRDS("./GBM_common_OS_plus_minus_genes_CGGA_325_693_TCGA_Slovenia.rds")
...
saveRDS(my_list_all, "GBM_common_PFS_plus_minus_genes_TCGA_Slovenia.rds")
# my_list_all <- readRDS("./GBM_common_PFS_plus_minus_genes_TCGA_Slovenia.rds")
...
test_randomness_2 = function(genes_1, genes_2, number_1, number_2, number) {
  n = c()
  for (i in c(1:1000)) {
  ...
  pval <- (sum(n >= number) / length(n))
  print(paste("p-values is", pval))
  return(pval)
}
...
test_randomness_4 = function(genes_1, genes_2, genes_3, genes_4, number_1, number_2, number_3, number_4, number) {
  n = c()
  for (i in c(1:1000)) {
    n = c(n, length(intersect(intersect(sample(genes_3, number_3), intersect(sample(genes_1, number_1), sample(genes_2, number_2))), sample(genes_4, number_4))))
    # print(length(intersect(sample(genes_3, number_3), intersect(sample(genes_1, number_1), sample(genes_2, number_2)))))
  }
  pval <- (sum(n >= number) / length(n))
  print(paste("p-values is", pval))
...
test_randomness_2_sets = function(genes_1, genes_2, n_gene_intersect) {
  number_1 <- length(genes_1)
  number_2 <- length(genes_2)
  n = c()
  for (i in c(1:1000)) {
    n = c(n, length(intersect(sample(genes_1, number_1), sample(genes_2, number_2))))
    # print(length(intersect(sample(genes_1, number_1), sample(genes_2, number_2))))
  }
  pval <- (sum(n >= n_gene_intersect) / length(n))
  print(paste("p-values is", pval))
  return(pval)
}

test_randomness_3_sets = function(genes_1, genes_2, genes_3, n_gene_intersect) {
  number_1 <- length(genes_1)
  number_2 <- length(genes_2)
  number_3 <- length(genes_3)
  n = c()
  for (i in c(1:1000)) {
    n = c(n, length(intersect(sample(genes_3, number_3), intersect(sample(genes_1, number_1), sample(genes_2, number_2)))))
    # print(length(intersect(sample(genes_3, number_3), intersect(sample(genes_1, number_1), sample(genes_2, number_2)))))
...
test_randomness_4(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS, my_list_plus$TCGA_GBM_OS, 1143 + 1, 405 + 1, 471 + 1, 900 + 1, 1)
test_randomness_3(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS, 1143 + 28, 405 + 28, 471 + 28, 28)
test_randomness_3(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$TCGA_GBM_OS, 1143 + 9, 405 + 9, 900 + 9, 9)
test_randomness_2(my_list_plus$TCGA_GBM_OS, my_list_plus$Slovenia_GBM_OS, 900 + 6, 471 + 6, 6)

test_randomness_2(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, 1143 + 72, 405 + 72, 72)
test_randomness_2(my_list_plus$CGGA_325_GBM_OS, my_list_plus$Slovenia_GBM_OS, 1143 + 14, 471 + 14, 14)
# test_randomness_2(my_list_plus$TCGA_GBM_OS, my_list_plus$Slovenia_GBM_OS, 1138 + 6, 132 + 6, 6)

test_randomness_2(my_list_plus$CGGA_325_GBM_OS, my_list_plus$Slovenia_GBM_OS, 1143 + 14, 471 + 14, 14)
test_randomness_2(my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS, 405 + 10, 471 + 10, 10)

test_randomness_2(my_list_plus$CGGA_325_GBM_OS, my_list_plus$TCGA_GBM_OS, 1143 + 72, 900 + 72, 72)
test_randomness_2(my_list_plus$CGGA_693_GBM_OS, my_list_plus$TCGA_GBM_OS, 405 + 15, 900 + 15, 15)

test_randomness_2(my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS, 405 + 10, 471 + 10, 10)

test_randomness_3(my_list_minus$CGGA_325_GBM_OS, my_list_minus$CGGA_693_GBM_OS, my_list_minus$Slovenia_GBM_OS, 868 + 1, 717 + 1, 352 + 1, 1)
test_randomness_3(my_list_minus$CGGA_325_GBM_OS, my_list_minus$CGGA_693_GBM_OS, my_list_minus$TCGA_GBM_OS, 868 + 3, 717 + 3, 386 + 3, 3)
test_randomness_2(my_list_minus$TCGA_GBM_OS, my_list_minus$Slovenia_GBM_OS, 386 + 5, 352 + 5, 5)
test_randomness_2(my_list_minus$TCGA_GBM_OS, my_list_minus$CGGA_325_GBM_OS, 386 + 12, 868 + 12, 12)
test_randomness_2(my_list_minus$CGGA_693_GBM_OS, my_list_minus$Slovenia_GBM_OS, 717 + 5, 352 + 5, 5)
test_randomness_2(my_list_minus$CGGA_693_GBM_OS, my_list_minus$TCGA_GBM_OS, 717 + 7, 386 + 7, 7)
test_randomness_2(my_list_minus$CGGA_325_GBM_OS, my_list_minus$Slovenia_GBM_OS, 868 + 14, 352 + 14, 14)
test_randomness_2(my_list_minus$CGGA_325_GBM_OS, my_list_minus$CGGA_693_GBM_OS, 868 + 137, 717 + 137, 137)

test_randomness_2(my_list_plus$TCGA_GBM_OS, my_list_plus$Slovenia_GBM_OS, 1124 + 20, 476 + 20, 20)
test_randomness_2(my_list_minus$TCGA_GBM_OS, my_list_minus$Slovenia_GBM_OS, 533 + 14, 361 + 14, 14)
...
TCGA_CGGA_325_693_common_plus_genes <- Reduce(intersect, list(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$TCGA_GBM_OS))
TCGA_CGGA_325_693_common_plus_genes_data <- list(genes <- TCGA_CGGA_325_693_common_plus_genes,
                                                 CGGA_325 <- final_CGGA_325_OS[final_CGGA_325_OS$gene %in% TCGA_CGGA_325_693_common_plus_genes,],
                                                 CGGA_693 <- final_CGGA_693_OS[final_CGGA_693_OS$gene %in% TCGA_CGGA_325_693_common_plus_genes,],
                                                 TCGA <- final_TCGA_OS[final_TCGA_OS$gene %in% TCGA_CGGA_325_693_common_plus_genes,])
saveRDS(TCGA_CGGA_325_693_common_plus_genes_data, "TCGA_CGGA_325_693_common_plus_genes.rds")

# Slovenia_CGGA_325_693_common_plus_genes <- Reduce(intersect, list(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS))
# Slovenia_CGGA_325_693_common_plus_genes_data <- list(genes <- Slovenia_CGGA_325_693_common_plus_genes,
#                                                  CGGA_325 <- final_CGGA_325_OS[final_CGGA_325_OS$gene %in% Slovenia_CGGA_325_693_common_plus_genes,],
#                                                  CGGA_693 <- final_CGGA_693_OS[final_CGGA_693_OS$gene %in% Slovenia_CGGA_325_693_common_plus_genes,],
#                                                  Slovenia <- final_Slovenia_OS[final_Slovenia_OS$gene %in% Slovenia_CGGA_325_693_common_plus_genes,])
# saveRDS(Slovenia_CGGA_325_693_common_plus_genes_data, "Slovenia_CGGA_325_693_common_plus_genes.rds")

Slovenia_CGGA_325_common_plus_genes <- Reduce(intersect, list(my_list_plus$CGGA_325_GBM_OS, my_list_plus$Slovenia_GBM_OS))
Slovenia_CGGA_693_common_plus_genes <- Reduce(intersect, list(my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS))


ggplot() + aes(as.numeric(c(dat2$CNR))) + geom_histogram(binwidth = 2, colour = "black", fill = "white")
...
# final_CGGA_OS = na.omit(final_CGGA_OS)
final_CGGA_325_OS = read_excel('CGGA_325_OS_HRs_pathways_third.xlsx')
final_CGGA_325_OS = na.omit(final_CGGA_325_OS)
final_CGGA_693_OS = read_excel('CGGA_693_OS_HRs_pathways_third.xlsx')
final_CGGA_693_OS = na.omit(final_CGGA_693_OS)
...
final_Slovenia_OS = read_excel('Slovenia_primary_GBM_PFS_HRs_pathways_third - test.xlsx')
# final_Slovenia_OS = read_excel('Slovenia_primary_GBM_OS_HRs_pathways_third - test.xlsx')
final_Slovenia_OS = na.omit(final_Slovenia_OS)
...
# a = read_excel('CGGA_325_counts_primary_GBM_OS_pathways_good_bad.xlsx')
colnames(final_CGGA_325_OS)[1] = 'pathways'
# final_CGGA_325_OS = left_join(final_CGGA_325_OS, a, by = 'pathways')
final_CGGA_325_OS = final_CGGA_325_OS[(final_CGGA_325_OS$log_rank < 0.05) & (final_CGGA_325_OS$HR_p < 0.05),]
# final_CGGA_325_OS = final_CGGA_325_OS[order(final_CGGA_325_OS$Tumour_Bad),]
# final_CGGA_325_OS = rbind(final_CGGA_325_OS[1:10,], final_CGGA_325_OS[(nrow(final_CGGA_325_OS) - 9):nrow(final_CGGA_325_OS),])
...
# a = read_excel('CGGA_counts_primary_GBM_PFS_pathways_good_bad.xlsx')
# a = read_excel('CGGA_693_counts_primary_GBM_OS_pathways_good_bad.xlsx')
colnames(final_CGGA_693_OS)[1] = 'pathways'
# final_CGGA_693_OS = left_join(final_CGGA_693_OS, a, by = 'pathways')
final_CGGA_693_OS = final_CGGA_693_OS[(final_CGGA_693_OS$log_rank < 0.05) & (final_CGGA_693_OS$HR_p < 0.05),]
# final_CGGA_693_OS = final_CGGA_693_OS[order(final_CGGA_693_OS$Tumour_Bad),]
# final_CGGA_693_OS = rbind(final_CGGA_693_OS[1:10,], final_CGGA_693_OS[(nrow(final_CGGA_693_OS) - 9):nrow(final_CGGA_693_OS),])
...
# a = read_excel('Mainz_counts_primary_GBM_OS_pathways_good_bad.xlsx')
# colnames(a)[1] = 'pathways'
# final_Mainz_OS = left_join(final_Mainz_OS, a, by = 'pathways')
# final_Mainz_OS = final_Mainz_OS[(final_Mainz_OS$log_rank < 0.05) & (final_Mainz_OS$HR_p < 0.05),]
# final_Mainz_OS = na.omit(final_Mainz_OS[order(final_Mainz_OS$Tumour_Bad),])
# final_Mainz_OS = rbind(final_Mainz_OS[1:10,], final_Mainz_OS[(nrow(final_Mainz_OS) - 9):nrow(final_Mainz_OS),])
...
final_Slovenia_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR_p < 0.05),]
# final_Slovenia_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05),]
# final_Slovenia_OS = na.omit(final_Slovenia_OS[order(final_Slovenia_OS$Tumour_Bad),])
# final_Slovenia_OS = rbind(final_Slovenia_OS[1:10,], final_Slovenia_OS[(nrow(final_Slovenia_OS) - 9):nrow(final_Slovenia_OS),])

pathways_TCGA = final_TCGA_OS$pathways
pathways_CGGA_325 = final_CGGA_325_OS$pathways
pathways_CGGA_693 = final_CGGA_693_OS$pathways
pathways_Slovenia = final_Slovenia_OS$pathways
# pathways_Mainz = final_Mainz_OS$pathways
# pathways = final_Mainz_OS$pathways

library(venn)
# OS #

my_list_plus = list(CGGA_325_GBM_OS = final_CGGA_325_OS[(final_CGGA_325_OS$log_rank < 0.05) & (final_CGGA_325_OS$HR_p < 0.05) & (final_CGGA_325_OS$HR > 0), 'pathways']$pathways,
                  CGGA_693_GBM_OS = final_CGGA_693_OS[(final_CGGA_693_OS$log_rank < 0.05) & (final_CGGA_693_OS$HR_p < 0.05) & (final_CGGA_693_OS$HR > 0), 'pathways']$pathways,
                   Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR_p < 0.05) & (final_Slovenia_OS$HR > 0), 'pathways']$pathways,
                   TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$log_rank < 0.05) & (final_TCGA_OS$HR_p < 0.05) & (final_TCGA_OS$HR > 0), 'pathways']$pathways)
my_list_minus = list(CGGA_325_GBM_OS = final_CGGA_325_OS[(final_CGGA_325_OS$log_rank < 0.05) & (final_CGGA_325_OS$HR_p < 0.05) & (final_CGGA_325_OS$HR < 0), 'pathways']$pathways,
                   CGGA_693_GBM_OS = final_CGGA_693_OS[(final_CGGA_693_OS$log_rank < 0.05) & (final_CGGA_693_OS$HR_p < 0.05) & (final_CGGA_693_OS$HR < 0), 'pathways']$pathways,
                   Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR_p < 0.05) & (final_Slovenia_OS$HR < 0), 'pathways']$pathways,
                   TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$log_rank < 0.05) & (final_TCGA_OS$HR_p < 0.05) & (final_TCGA_OS$HR < 0), 'pathways']$pathways)

my_list_all = list(CGGA_325 = c(my_list_plus$CGGA_325_GBM_OS, my_list_minus$CGGA_325_GBM_OS),
                 CGGA_693 = c(my_list_plus$CGGA_693_GBM_OS, my_list_minus$CGGA_693_GBM_OS),
                 Slovenia = c(my_list_plus$Slovenia_GBM_OS, my_list_minus$Slovenia_GBM_OS),
                 TCGA = c(my_list_plus$TCGA_GBM_OS, my_list_minus$TCGA_GBM_OS))

# my_list_all = list(TCGA = c(my_list_plus$TCGA_GBM_OS, my_list_minus$TCGA_GBM_OS),
#                  CGGA_GBM = c(my_list_plus$CGGA_GBM_OS, my_list_minus$CGGA_GBM_OS),
#                  Slovenia = c(my_list_plus$Slovenia_GBM_OS, my_list_minus$Slovenia_GBM_OS))
# saveRDS(my_list_all, "GBM_common_OS_plus_minus_pathways_CGGA_325_693_TCGA_Slovenia - test.rds")
saveRDS(my_list_all, "GBM_common_OS_plus_minus_pathways_CGGA_325_693_TCGA_Slovenia.rds")
# my_list_all <- readRDS("./GBM_common_OS_plus_minus_pathways_CGGA_325_693_TCGA_Slovenia.rds")

# PFS #

my_list_plus = list(TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$log_rank < 0.05) & (final_TCGA_OS$HR_p < 0.05) & (final_TCGA_OS$HR > 0), 'pathways']$pathways,
                   Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR_p < 0.05) & (final_Slovenia_OS$HR > 0), 'pathways']$pathways)
my_list_minus = list(TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$log_rank < 0.05) & (final_TCGA_OS$HR_p < 0.05) & (final_TCGA_OS$HR < 0), 'pathways']$pathways,
                   Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR_p < 0.05) & (final_Slovenia_OS$HR < 0), 'pathways']$pathways)

# my_list_plus = list(TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$log_rank < 0.05) & (final_TCGA_OS$HR_p < 0.05) & (final_TCGA_OS$HR > 0), 'pathways']$pathways,
#                    Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR > 0), 'pathways']$pathways)
# my_list_minus = list(TCGA_GBM_OS = final_TCGA_OS[(final_TCGA_OS$log_rank < 0.05) & (final_TCGA_OS$HR_p < 0.05) & (final_TCGA_OS$HR < 0), 'pathways']$pathways,
#                    Slovenia_GBM_OS = final_Slovenia_OS[(final_Slovenia_OS$log_rank < 0.05) & (final_Slovenia_OS$HR < 0), 'pathways']$pathways)
...
saveRDS(my_list_all, "GBM_common_PFS_plus_minus_pathways_TCGA_Slovenia.rds")
# my_list_all <- readRDS("./GBM_common_PFS_plus_minus_pathways_TCGA_Slovenia.rds")
...
cat(intersect(my_list_minus$TCGA_GBM_OS, my_list_minus$Mainz_GBM_OS), sep = "\n")
pathways <- c(intersect(my_list_minus$TCGA_GBM_OS, my_list_minus$Mainz_GBM_OS),
        intersect(my_list_plus$TCGA_GBM_OS, my_list_plus$Mainz_GBM_OS))
...
final_TCGA_OS_short <- final_TCGA_OS[!is.na(match(final_TCGA_OS$pathways, our_pathways)),]
final_CGGA_OS_short <- final_CGGA_OS[!is.na(match(final_CGGA_OS$pathways, our_pathways)),]
final_Slovenia_OS_short <- final_Slovenia_OS[!is.na(match(final_Slovenia_OS$pathways, our_pathways)),]

Reduce(intersect, my_list_plus)


test_randomness_2 = function(pathways_1, pathways_2, number_1, number_2, number) {
  n = c()
  for (i in c(1:1000)) {
    n = c(n, length(intersect(sample(pathways_1, number_1), sample(pathways_2, number_2))))
    # print(length(intersect(sample(pathways_1, number_1), sample(pathways_2, number_2))))
  }
  pval <- (sum(n >= number) / length(n))
  print(paste("p-values is", pval))
  return(pval)
...
test_randomness_3 = function(pathways_1, pathways_2, pathways_3, number_1, number_2, number_3, number) {
  n = c()
  for (i in c(1:1000)) {
    n = c(n, length(intersect(sample(pathways_3, number_3), intersect(sample(pathways_1, number_1), sample(pathways_2, number_2)))))
    # print(length(intersect(sample(pathways_3, number_3), intersect(sample(pathways_1, number_1), sample(pathways_2, number_2)))))
  }
  pval <- (sum(n >= number) / length(n))
  print(paste("p-values is", pval))
  return(pval)
...
test_randomness_2_sets = function(pathways_1, pathways_2, n_gene_intersect) {
  number_1 <- length(pathways_1)
  number_2 <- length(pathways_2)
  n = c()
  for (i in c(1:1000)) {
    n = c(n, length(intersect(sample(pathways_1, number_1), sample(pathways_2, number_2))))
    # print(length(intersect(sample(pathways_1, number_1), sample(pathways_2, number_2))))
...
test_randomness_3_sets = function(pathways_1, pathways_2, pathways_3, n_gene_intersect) {
  number_1 <- length(pathways_1)
  number_2 <- length(pathways_2)
  number_3 <- length(pathways_3)
  n = c()
  for (i in c(1:1000)) {
    n = c(n, length(intersect(sample(pathways_3, number_3), intersect(sample(pathways_1, number_1), sample(pathways_2, number_2)))))
    # print(length(intersect(sample(pathways_3, number_3), intersect(sample(pathways_1, number_1), sample(pathways_2, number_2)))))
...
# test_randomness_3(pathways_TCGA, pathways_CGGA, pathways_Slovenia, 1270, 1355, 958, 1)

# test_randomness_3(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS, 345+4, 92+4, 10+4, 4)
# test_randomness_3(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$TCGA_GBM_OS, 345+27, 92+27, 19+27, 27)
test_randomness_3(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$TCGA_GBM_OS,
                  322 + 1, 92 + 1, 63 + 1, 1)
test_randomness_3(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS,
                  322 + 1, 92 + 1, 5 + 1, 1)
test_randomness_2(my_list_plus$CGGA_325_GBM_OS, my_list_plus$Slovenia_GBM_OS,
                  322 + 3, 5 + 3, 3)
test_randomness_2(my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS,
                  92 + 1, 5 + 1, 1)
test_randomness_2(my_list_plus$TCGA_GBM_OS, my_list_plus$CGGA_325_GBM_OS,
                  63 + 47, 322 + 47, 47)
test_randomness_2(my_list_plus$TCGA_GBM_OS, my_list_plus$CGGA_693_GBM_OS,
                  63 + 1, 92 + 1, 1)
test_randomness_2(my_list_plus$CGGA_325_GBM_OS, my_list_plus$CGGA_693_GBM_OS,
                    322 + 23, 92 + 23, 23)

test_randomness_2(my_list_minus$TCGA_GBM_OS, my_list_minus$Slovenia_GBM_OS,
                  11 + 1, 24 + 1, 1)

test_randomness_2(my_list_minus$TCGA_GBM_OS, my_list_minus$CGGA_325_GBM_OS,
                  11 + 3, 118 + 3, 3)
test_randomness_2(my_list_minus$CGGA_693_GBM_OS, my_list_minus$CGGA_325_GBM_OS,
                  68 + 9, 118 + 9, 9)

Reduce(intersect, list(my_list_plus$TCGA_GBM_OS, my_list_plus$CGGA_325_GBM_OS))

# test_randomness_3(my_list_minus$CGGA_325_GBM_OS, my_list_minus$CGGA_693_GBM_OS, my_list_minus$Slovenia_GBM_OS, 345+1, 92 +1, 352+1, 1)
# test_randomness_3(my_list_minus $CGGA_325_GBM_OS, my_list_minus$CGGA_693_GBM_OS, my_list_minus$TCGA_GBM_OS,  345+3, 92+3, 386+3, 3)
# test_randomness_2(my_list_minus$TCGA_GBM_OS, my_list_minus$Slovenia_GBM_OS, 386+5, 352+5, 5)

# test_randomness_3_sets(pathways_CGGA_325, pathways_CGGA_693, pathways_Slovenia,)
# test_randomness_3_sets(pathways_CGGA_325, pathways_CGGA_693, pathways_TCGA,)
# test_randomness_2_sets(pathways_TCGA, pathways_Slovenia, 6)

a <- test_randomness(pathways_TCGA, pathways_Mainz, 547, 332, 13)
a <- test_randomness(pathways, pathways, 17, 14, 3)

length(final_TCGA_OS[(final_TCGA_OS$pvalue < 0.05) & (final_TCGA_OS$HR < 0), 'gene']$gene)

Reduce(intersect, list(my_list_plus$CGGA_325_GBM_OS, my_list_plus$TCGA_GBM_OS))
...
CGGA_325 <- read_csv('counts_CGGA_325_GBM_OS.csv')
CGGA_693 <- read_csv('counts_CGGA_693_GBM_OS.csv')
...
all <- merge(CGGA_693, CGGA_325, by = "X1")
all <- merge(all, TCGA, by = "X1")
all <- merge(all, Slovenia, by = "X1")
...
metadata <- data.frame(name = colnames(all),
                        dataset = c(rep('CGGA_693', ncol(CGGA_693) - 1),
                                  rep('CGGA_325', ncol(CGGA_325) - 1),
                                  rep('TCGA', ncol(TCGA) - 1),
                                  rep('Slovenia', ncol(Slovenia) - 1)))
...
export_analysis_plot(filename = "pca_plot_by_gene_exp",
                     plot = pca_plot_by_gene_exp + scale_four_colour_Publication(),
...
export_analysis_plot(filename = "pca_plot_by_pathways_scores",
                     plot = pca_plot_by_pathways_scores + xlim(-100, 100) + ylim(-100, 100) + scale_four_colour_Publication(),
                     path = "C:/Users/raevs/Desktop/Oncobox/LTS_vs_STS_manuscript/plots/pca",
                     scale = 1,
                     width = 210 / 1.85,
                     height = 297 / 2.65,
...
final_CGGA_325_OS = read_excel('CGGA_325_OS_HRs_genes_third.xlsx')
final_CGGA_325_OS$log10_log_rank = log10(final_CGGA_325_OS$log_rank)
final_CGGA_325_OS = na.omit(final_CGGA_325_OS)

final_CGGA_693_OS = read_excel('CGGA_693_OS_HRs_genes_third.xlsx')
final_CGGA_693_OS$log10_log_rank = log10(final_CGGA_693_OS$log_rank)
final_CGGA_693_OS = na.omit(final_CGGA_693_OS)
...
EnhancedVolcano(final_CGGA_325_OS,
                x = 'HR',
                y = 'log_rank',
                lab = final_CGGA_325_OS$gene,
                pCutoff = 0.05, FCcutoff = 30)

EnhancedVolcano(final_CGGA_693_OS,
                x = 'HR',
                y = 'log_rank',
                lab = final_CGGA_325_OS$gene,
                pCutoff = 0.05, FCcutoff = 30)
...
pdf("Volcano_CGGA_325_OS_short.pdf", height = 8, width = 8)
EnhancedVolcano(final_CGGA_325_OS,
                x = 'HR',
                y = 'log_rank',
                lab = final_CGGA_325_OS$gene,
                  pCutoff = 0.05, FCcutoff = 30, ylim = c(0, 6), xlim = c(-2.5, 2.5))
dev.off()

pdf("Volcano_CGGA_325_OS.pdf", height = 8, width = 8)
EnhancedVolcano(final_CGGA_325_OS,
                x = 'HR',
                y = 'log_rank',
                lab = final_CGGA_325_OS$gene,
...
pdf("Volcano_CGGA_693_OS_short.pdf", height = 8, width = 8)
EnhancedVolcano(final_CGGA_693_OS,
                x = 'HR',
                y = 'lo g_rank',
                lab = final_CGGA_693_OS$gene,
                  pCutoff = 0.05, FCcutoff = 30, ylim = c(0, 5), xlim = c(-5, 5))
dev.off()

pdf("Volcano_CGGA_693_OS.pdf", height = 8, width = 8)
EnhancedVolcano(final_CGGA_693_OS,
                x = 'HR',
                y = 'log_rank',
                lab = final_CGGA_693_OS$gene,
...
pdf("Volcano_Slovenia_PFS_short - test.pdf", height = 8, width = 8)
EnhancedVolcano(final_Slovenia_OS,
                x = 'HR',
                y = 'log_rank',
                lab = final_Slovenia_OS$gene,
...
pdf("Volcano_Slovenia_PFS - test.pdf", height = 8, width = 8)
EnhancedVolcano(final_Slovenia_OS,
                x = 'HR',
                y = 'log_rank',
                lab = final_Slovenia_OS$gene,
...
genes_TCGA_Mainz = c(intersect(my_list_minus$TCGA_GBM_OS, my_list_minus$Mainz_GBM_OS),
 intersect(my_list_plus$TCGA_GBM_OS, my_list_plus$M ainz_GBM_OS))
...
TCGA_CGGA_325_common_plus_genes <- Reduce(intersect, list(my_list_plus$CGGA_325_GBM_OS, my_list_plus$TCGA_GBM_OS))
TCGA_CGGA_693_common_plus_genes <- Reduce(intersect, list(my_list_plus$CGGA_693_GBM_OS, my_list_plus$TCGA_GBM_OS))
TCGA_CGGA_325_693_intersect_plus_genes <- intersect(TCGA_CGGA_325_common_plus_genes, TCGA_CGGA_693_common_plus_genes)

# TCGA_CGGA_325_693_intersect_plus_genes
# [1] "MXRA5" "MARVELD1" "LBH" "SLC43A3" "CKAP4" "PDIA4" "FAM20C" "AEBP1" "PDAP1"

# Slovenia_CGGA_325_common_plus_genes <- Reduce(intersect, list(my_list_plus$CGGA_325_GBM_OS, my_list_plus$Slovenia_GBM_OS))
# Slovenia_CGGA_693_common_plus_genes <- Reduce(intersect, list(my_list_plus$CGGA_693_GBM_OS, my_list_plus$Slovenia_GBM_OS))
# Slovenia_CGGA_325_693_common_plus_genes <- unique(c(Slovenia_CGGA_325_common_plus_genes, Slovenia_CGGA_693_common_plus_genes))


entrez = as.character(hgnc[!is.na(match(hgnc$symbol, genes_CGGA_TCGA_OS)), "entrez_id"]$entrez_id)
entrez = as.character(hgnc[!is.na(match(hgnc$symbol, Slovenia_CGGA_325_693_common_plus_genes)), "entrez_id"]$entrez_id)
# entrez = as.character(hgnc[!is.na(match(hgnc$symbol, TCGA_CGGA_325_693_common_plus_genes)), "entrez_id"]$entrez_id)
go_enrichment <- enrichGO(entrez, OrgDb = "org.Hs.eg.db", ont = "ALL", readable = TRUE)

pdf("enrich_plot_TCGA_CGGA_325_693_OS.pdf", height = 4, width = 8)
pdf("enrich_plot_Slovenia_CGGA_325_693_OS.pdf", height = 4, width = 8)
dotplot(go_enrichment, showCategory = 100)
dev.off()
